...
[[ -z $APP || -z $PORT || -z $DB_HOST || -z $DB_PORT || \
   -z $DB_USER || -z $DB_PASSWORD || -z $DB_NAME || -z $BASE_URL ]] && usage

OUT_DIR="$APP"
rm -rf "$OUT_DIR"
mkdir -p "$OUT_DIR"/k8s

export APP PORT DB_HOST DB_PORT DB_USER DB_PASSWORD DB_NAME BASE_URL
        ...
        stage('Deploy') {
            steps {
                withCredentials([
                    ...
                    string(credentialsId: 'GATEWAY_URL_ID', variable: 'BASE_URL'),
